...
'___'
# - テキスト入力に `2501` と入れると、数値的な解がオレンジの点線で表示されます。  
...
if mode == "手動最適化":
    with col1:
        ...
        X= st.number_input(label="求める$x$座標の手動入力"
                            ...
                            , key="number_input_X")
        T = time_T(X)
        output_text = f"点$\\rm P$のx座標 = {X:.3f},   移動時間 T = {T:.4f}"
        # 経路プロット
        fig_path = go.Figure()
        fig_path.add_trace(go.Scatter(x=[0, X], y=[y_start, 0], mode='lines+markers', name='媒質1'))
        fig_path.add_trace(go.Scatter(x=[X, xt], y=[0, yt], mode='lines+markers', name='媒質2'))
    ...
    with col2:
        ...
        fig_time.update_layout(title="交点 X による移動時間の履歴", xaxis_title="交点 X", yaxis_title="移動時間 T", width=400, height=400)
        ...
        if st.session_state.history_T:
            ...
            st.write(f"最小移動時間 T = {min_T:.4f} （X = {min_X:.3f}）") 
    ...
    if st.button("Start Optimization"):
        ...
        # for i, (X_val, T_val) in enumerate(zip(hist_X, hist_T)):
        #     st.write(f"Step {i}: X = {X_val:.6f}, T = {T_val:.6f}")
